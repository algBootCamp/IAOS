...
# print(basicindexdata.head(3))
# 所属行业
# {
#     行业1:[ts_code1,ts_code2,......],
#     行业2:[ts_code1,ts_code2,......]
# }
# 行业股票字典
industry_map = dict()
# 行业
...
def partition_cap():
    ...
    划分大盘、中小盘
    """
    df = tsdatacapture.get_daily_basic(start_date='20220909')
    data_max, data_min, valuation_low, valuation_mid, valuation_high = get_data_percentile(
        np.array(df.iloc[:].loc[:, 'float_share']).tolist(), 50, 75, 90)
    for index, row in df.iterrows():
        float_share = row["float_share"]
        ts_code = row["ts_code"]
        if float_share <= valuation_low:
            small_cap_stocks.append(ts_code)
        elif float_share >= valuation_high:
            large_cap_stocks.append(ts_code)
        else:
            mid_cap_stocks.append(ts_code)
    print(len(small_cap_stocks), len(mid_cap_stocks), len(large_cap_stocks), df["float_share"].size)
    # print(data_max * billion, data_min * billion, valuation_low * billion, valuation_mid * billion,valuation_high * billion)
